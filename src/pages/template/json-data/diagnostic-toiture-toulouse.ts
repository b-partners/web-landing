export default {
  route: 'paris',
  metaTitle: 'Diagnostic toiture à Toulouse : un contrôle adapté aux conditions climatiques locales',
  metaDescription:
    'Découvrez pourquoi réaliser un diagnostic toiture à Toulouse est essentiel, ses étapes et ses bénéfices pour la durabilité et la sécurité de votre bâtiment.',
  hero: {
    title: 'Diagnostic toiture à Toulouse : un contrôle adapté aux conditions climatiques locales',
    items: [
      'Un diagnostic toiture Toulouse évalue l’état de la couverture et de la charpente.',
      'Il détecte infiltrations, fissures, usures et défauts liés au climat de Haute-Garonne.',
      'Recommandé avant une vente, après intempéries ou dans le cadre d’un entretien préventif.',
    ],
  },
  testimonial: { title1: 'Témoignages clients', title2: 'Passez à l’analyse intelligente, sans compléxité' },
  testimonials: [
    {
      name: 'Arnaud P.',
      star: 5,
      content:
        '"L’outil Birdia est simple, rapide et fiable. Je peux générer un rapport technique avant même de visiter le chantier. Cela me fait gagner des heures chaque semaine, tout en améliorant la qualité de mes prestations."',
      job: 'Gérant entreprise de couverture (Cannes)',
    },
    {
      star: 5,
      name: 'Joël D.',
      content:
        'Birdia, c’est mon assistant digital. Je l’utilise au quotidien pour détecter les toits à rénover, et les clients adorent recevoir un rapport visuel clair. Même mes devis sont mieux acceptés grâce à ça.',
      job: 'Couvreur expérimenté (Hauts-de-France)',
    },
    {
      name: 'Laurent M.',
      star: 5,
      content:
        'Birdia est un véritable game changer. L’outil est à la fois intuitif, puissant et ludique. En quelques clics, nous pouvons visualiser, détecter et évaluer des centaines de toitures. Cela change notre manière de penser la prévention et le service client.',
      job: 'Responsable Innovation – Assureur Habitation',
    },
    {
      star: 5,
      name: 'Émilie C.',
      content:
        'Grâce à Birdia, nos équipes disposent d’une vision détaillée de chaque bien assuré. La précision de l’analyse des matériaux, de l’usure et des risques nous permet d’adapter finement les contrats. C’est un vrai plus pour la maîtrise technique et commerciale.',
      job: 'Responsable Souscription Habitation – Groupe IARD',
    },
  ],
  analyse: {
    title: 'Analyse automatisée de toitures par intelligence artificielle',
    information: [
      {
        title: 'Pourquoi le diagnostic toiture à Toulouse est essentiel ?',
        description: 'Il garantit une vérification adaptée aux fortes chaleurs estivales et aux orages fréquents.',
      },
      {
        title: 'Comment se déroule le diagnostic toiture à Toulouse ?',
        description: 'Inspection visuelle, contrôle d’étanchéité, vérification de l’isolation et rapport technique.',
      },
      {
        title: 'Que faire après un diagnostic toiture à Toulouse ?',
        description: 'Programmer réparations, renforcer l’isolation ou planifier un suivi périodique adapté au climat.',
      },
    ],
    explication:
      ' Détection, qualification et recommandation à partir d’images aériennes HD. En un clic, obtenez un diagnostic métier précis sans monter sur le toit.',
  },
  theyTrustUs: { title: 'Ils nous font confiance' },
  faq: {
    title1: 'FAQ',
    q1: { question: 'Question 1', answer: 'Réponse 1' },
    q2: { question: 'Question 2', answer: 'Réponse 2' },
    q3: { question: 'Question 3', answer: 'Réponse 3' },
    q4: { question: 'Question 4', answer: 'Réponse 4' },
    q5: { question: 'Question 5', answer: 'Réponse 5' },
    q6: { question: 'Question 6', answer: 'Réponse 6' },
    title2: 'Vous êtes un particulier , vous souhaitez trouver un couvreur ?',
  },
  roofDiagnostics: {
    title: 'Comprendre les enjeux long terme de diagnostic toiture à Toulouse',
    description1:
      'Un diagnostic toiture à Toulouse est une démarche préventive qui permet de protéger durablement son patrimoine immobilier. Les toitures toulousaines subissent des contraintes spécifiques : chaleur estivale, épisodes orageux violents et parfois même grêle. Ce contrôle technique permet de repérer rapidement les défauts d’étanchéité, les tuiles endommagées ou les faiblesses de charpente. ',
    description2:
      'Sur le long terme, il contribue à prolonger la durée de vie du toit, à éviter des réparations coûteuses et à garantir la sécurité des occupants. Intégré dans un entretien régulier, il assure un confort optimal et une meilleure résistance face aux aléas climatiques locaux.',
  },
  footer: {
    title: 'Votre analyse Birdia près de chez vous',
    description: 'Découvrez nos offres disponibles dans les villes suivantes :',
    link: [
      { name: 'paris', route: '/paris' },
      { name: 'Lille', route: '/lille' },
      { name: 'Lyon', route: '/Lyon' },
      { name: 'paris', route: '/paris' },
      { name: 'paris', route: '/paris' },
      { name: 'paris', route: '/paris' },
      { name: 'paris', route: '/paris' },
      { name: 'paris', route: '/paris' },
      { name: 'paris', route: '/paris' },
    ],
  },
};
