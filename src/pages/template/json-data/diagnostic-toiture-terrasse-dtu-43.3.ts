export default {
  route: 'paris',
  metaTitle: 'Diagnostic toiture terrasse DTU 43.3 : un contrôle conforme aux normes professionnelles',
  metaDescription:
    'Découvrez pourquoi réaliser un diagnostic toiture terrasse DTU 43.3 est indispensable, ses étapes et son rôle dans la conformité réglementaire.',
  hero: {
    title: 'Diagnostic toiture terrasse DTU 43.3 : un contrôle conforme aux normes professionnelles',
    items: [
      'Le diagnostic toiture terrasse DTU 43.3 s’appuie sur une norme encadrant l’étanchéité des toitures-terrasses.',
      'Il garantit une vérification technique rigoureuse et adaptée aux règles de construction.',
      'Recommandé pour les bâtiments existants et lors de projets de rénovation.',
    ],
  },
  testimonial: { title1: 'Témoignages clients', title2: 'Passez à l’analyse intelligente, sans compléxité' },
  testimonials: [
    {
      name: 'Arnaud P.',
      star: 5,
      content:
        '"L’outil Birdia est simple, rapide et fiable. Je peux générer un rapport technique avant même de visiter le chantier. Cela me fait gagner des heures chaque semaine, tout en améliorant la qualité de mes prestations."',
      job: 'Gérant entreprise de couverture (Cannes)',
    },
    {
      star: 5,
      name: 'Joël D.',
      content:
        'Birdia, c’est mon assistant digital. Je l’utilise au quotidien pour détecter les toits à rénover, et les clients adorent recevoir un rapport visuel clair. Même mes devis sont mieux acceptés grâce à ça.',
      job: 'Couvreur expérimenté (Hauts-de-France)',
    },
    {
      name: 'Laurent M.',
      star: 5,
      content:
        'Birdia est un véritable game changer. L’outil est à la fois intuitif, puissant et ludique. En quelques clics, nous pouvons visualiser, détecter et évaluer des centaines de toitures. Cela change notre manière de penser la prévention et le service client.',
      job: 'Responsable Innovation – Assureur Habitation',
    },
    {
      star: 5,
      name: 'Émilie C.',
      content:
        'Grâce à Birdia, nos équipes disposent d’une vision détaillée de chaque bien assuré. La précision de l’analyse des matériaux, de l’usure et des risques nous permet d’adapter finement les contrats. C’est un vrai plus pour la maîtrise technique et commerciale.',
      job: 'Responsable Souscription Habitation – Groupe IARD',
    },
  ],
  analyse: {
    title: 'Analyse automatisée de toitures par intelligence artificielle',
    information: [
      {
        title: 'Pourquoi le diagnostic toiture terrasse DTU 43.3 est essentiel ?',
        description: 'Il assure la conformité réglementaire et prévient les désordres liés aux infiltrations d’eau.',
      },
      {
        title: 'Comment se déroule le diagnostic toiture terrasse DTU 43.3 ?',
        description: 'Inspection des éléments porteurs, contrôle des relevés d’étanchéité et vérification des matériaux.',
      },
      {
        title: 'Que faire après diagnostic toiture terrasse DTU 43.3 ?',
        description: 'Appliquer les corrections nécessaires, planifier des réparations ou renforcer le système d’étanchéité.',
      },
    ],
    explication:
      ' Détection, qualification et recommandation à partir d’images aériennes HD. En un clic, obtenez un diagnostic métier précis sans monter sur le toit.',
  },
  theyTrustUs: { title: 'Ils nous font confiance' },
  faq: {
    title1: 'FAQ',
    q1: { question: 'Question 1', answer: 'Réponse 1' },
    q2: { question: 'Question 2', answer: 'Réponse 2' },
    q3: { question: 'Question 3', answer: 'Réponse 3' },
    q4: { question: 'Question 4', answer: 'Réponse 4' },
    q5: { question: 'Question 5', answer: 'Réponse 5' },
    q6: { question: 'Question 6', answer: 'Réponse 6' },
    title2: 'Vous êtes un particulier , vous souhaitez trouver un couvreur ?',
  },
  roofDiagnostics: {
    title: 'Comprendre les enjeux long terme de diagnostic toiture terrasse DTU 43.3',
    description1:
      'Un diagnostic toiture terrasse DTU 43.3 garantit que la couverture respecte les normes en vigueur. Ce contrôle technique s’appuie sur une réglementation précise définissant la mise en œuvre et la maintenance des toitures-terrasses accessibles ou non accessibles. Sur le long terme, il permet de limiter les risques d’infiltrations, de prolonger la durée de vie des matériaux et d’assurer une meilleure performance énergétique. ',
    description2:
      'Il contribue également à sécuriser la structure du bâtiment et à réduire les coûts d’entretien imprévus. En respectant le DTU 43.3, le diagnostic devient un véritable outil de fiabilité et de durabilité.',
  },
  footer: {
    title: 'Votre analyse Birdia près de chez vous',
    description: 'Découvrez nos offres disponibles dans les villes suivantes :',
    link: [
      { name: 'paris', route: '/paris' },
      { name: 'Lille', route: '/lille' },
      { name: 'Lyon', route: '/Lyon' },
      { name: 'paris', route: '/paris' },
      { name: 'paris', route: '/paris' },
      { name: 'paris', route: '/paris' },
      { name: 'paris', route: '/paris' },
      { name: 'paris', route: '/paris' },
      { name: 'paris', route: '/paris' },
    ],
  },
};
