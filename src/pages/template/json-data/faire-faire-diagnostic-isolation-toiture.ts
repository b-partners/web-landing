export default {
  route: '/faire-faire-diagnostic-isolation-toiture',
  metaTitle: 'Comment faire faire un diagnostic de l’isolation d’une toiture : méthode et bonnes pratiques',
  metaDescription:
    'Découvrez comment faire faire un diagnostic de l’isolation d’une toiture, les étapes à suivre et les bénéfices pour votre confort et vos économies d’énergie.',
  hero: {
    title: 'Comment faire faire un diagnostic de l’isolation d’une toiture : méthode et bonnes pratiques',
    items: [
      'Savoir comment faire faire un diagnostic de l’isolation d’une toiture permet d’optimiser les performances thermiques du bâtiment.',
      'Ce contrôle identifie pertes de chaleur, défauts d’isolation et zones nécessitant une amélioration.',
      'Il est recommandé avant rénovation énergétique ou pour réduire les dépenses de chauffage.',
    ],
  },
  testimonial: { title1: 'Témoignages clients', title2: 'Passez à l’analyse intelligente, sans compléxité' },
  testimonials: [
    {
      name: 'Arnaud P.',
      star: 5,
      content:
        '"L’outil Birdia est simple, rapide et fiable. Je peux générer un rapport technique avant même de visiter le chantier. Cela me fait gagner des heures chaque semaine, tout en améliorant la qualité de mes prestations."',
      job: 'Gérant entreprise de couverture (Cannes)',
    },
    {
      star: 5,
      name: 'Joël D.',
      content:
        'Birdia, c’est mon assistant digital. Je l’utilise au quotidien pour détecter les toits à rénover, et les clients adorent recevoir un rapport visuel clair. Même mes devis sont mieux acceptés grâce à ça.',
      job: 'Couvreur expérimenté (Hauts-de-France)',
    },
    {
      name: 'Laurent M.',
      star: 5,
      content:
        'Birdia est un véritable game changer. L’outil est à la fois intuitif, puissant et ludique. En quelques clics, nous pouvons visualiser, détecter et évaluer des centaines de toitures. Cela change notre manière de penser la prévention et le service client.',
      job: 'Responsable Innovation – Assureur Habitation',
    },
    {
      star: 5,
      name: 'Émilie C.',
      content:
        'Grâce à Birdia, nos équipes disposent d’une vision détaillée de chaque bien assuré. La précision de l’analyse des matériaux, de l’usure et des risques nous permet d’adapter finement les contrats. C’est un vrai plus pour la maîtrise technique et commerciale.',
      job: 'Responsable Souscription Habitation – Groupe IARD',
    },
  ],
  analyse: {
    title: 'Analyse automatisée de toitures par intelligence artificielle',
    information: [
      {
        title: 'Pourquoi faire un diagnostic de l’isolation d’une toiture est essentiel ?',
        description: 'Il permet de repérer les déperditions thermiques et d’améliorer le confort intérieur durablement.',
      },
      {
        title: 'Comment se déroule comment faire faire un diagnostic de l’isolation d’une toiture ?',
        description: 'Inspection visuelle, mesures thermographiques et analyse de l’efficacité des matériaux isolants.',
      },
      {
        title: 'Que faire après comment faire faire un diagnostic de l’isolation d’une toiture ?',
        description: 'Prévoir des travaux d’amélioration, renforcer l’isolation ou installer de nouveaux matériaux adaptés.',
      },
    ],
    explication:
      ' Détection, qualification et recommandation à partir d’images aériennes HD. En un clic, obtenez un diagnostic métier précis sans monter sur le toit.',
  },
  theyTrustUs: { title: 'Ils nous font confiance' },
  faq: {
    title1: 'FAQ',
    q1: { question: 'Question 1', answer: 'Réponse 1' },
    q2: { question: 'Question 2', answer: 'Réponse 2' },
    q3: { question: 'Question 3', answer: 'Réponse 3' },
    q4: { question: 'Question 4', answer: 'Réponse 4' },
    q5: { question: 'Question 5', answer: 'Réponse 5' },
    q6: { question: 'Question 6', answer: 'Réponse 6' },
    title2: 'Vous êtes un particulier , vous souhaitez trouver un couvreur ?',
  },
  roofDiagnostics: {
    title: 'Comprendre les enjeux long terme de comment faire faire un diagnostic de l’isolation d’une toiture',
    description1:
      'Réaliser un diagnostic de l’isolation d’une toiture permet de maîtriser durablement les performances énergétiques d’un bâtiment. Ce contrôle met en évidence les zones de déperdition et guide vers les solutions les plus adaptées. Sur le long terme, il contribue à réduire la consommation d’énergie, à améliorer le confort thermique et à diminuer l’empreinte carbone. ',
    description2:
      'Il permet également de prolonger la durée de vie des matériaux isolants et d’éviter les désordres liés à l’humidité. Un suivi régulier garantit un toit performant, économe et respectueux des normes en vigueur.',
  },
  footer: {
    title: 'Votre analyse Birdia près de chez vous',
    description: 'Découvrez nos offres disponibles dans les villes suivantes :',
    link: [
      { name: 'paris', route: '/paris' },
      { name: 'Lille', route: '/lille' },
      { name: 'Lyon', route: '/Lyon' },
      { name: 'paris', route: '/paris' },
      { name: 'paris', route: '/paris' },
      { name: 'paris', route: '/paris' },
      { name: 'paris', route: '/paris' },
      { name: 'paris', route: '/paris' },
      { name: 'paris', route: '/paris' },
    ],
  },
};
