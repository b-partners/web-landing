export default {
  route: 'paris',
  metaTitle: 'Diagnostic toiture terrasse PDF : un rapport clair et structuré pour vos toits plats',
  metaDescription:
    'Découvrez pourquoi un diagnostic toiture terrasse PDF est indispensable, son déroulement et son utilité pour le suivi technique et administratif.',
  hero: {
    title: 'Diagnostic toiture terrasse PDF : un rapport clair et structuré pour vos toits plats',
    items: [
      'Un diagnostic toiture terrasse PDF fournit un rapport détaillé et facilement partageable.',
      'Il inclut l’état de l’étanchéité, des isolants et de la structure porteuse.',
      'Utile pour la maintenance, la vente immobilière ou la planification de travaux.',
    ],
  },
  testimonial: { title1: 'Témoignages clients', title2: 'Passez à l’analyse intelligente, sans compléxité' },
  testimonials: [
    {
      name: 'Arnaud P.',
      star: 5,
      content:
        '"L’outil Birdia est simple, rapide et fiable. Je peux générer un rapport technique avant même de visiter le chantier. Cela me fait gagner des heures chaque semaine, tout en améliorant la qualité de mes prestations."',
      job: 'Gérant entreprise de couverture (Cannes)',
    },
    {
      star: 5,
      name: 'Joël D.',
      content:
        'Birdia, c’est mon assistant digital. Je l’utilise au quotidien pour détecter les toits à rénover, et les clients adorent recevoir un rapport visuel clair. Même mes devis sont mieux acceptés grâce à ça.',
      job: 'Couvreur expérimenté (Hauts-de-France)',
    },
    {
      name: 'Laurent M.',
      star: 5,
      content:
        'Birdia est un véritable game changer. L’outil est à la fois intuitif, puissant et ludique. En quelques clics, nous pouvons visualiser, détecter et évaluer des centaines de toitures. Cela change notre manière de penser la prévention et le service client.',
      job: 'Responsable Innovation – Assureur Habitation',
    },
    {
      star: 5,
      name: 'Émilie C.',
      content:
        'Grâce à Birdia, nos équipes disposent d’une vision détaillée de chaque bien assuré. La précision de l’analyse des matériaux, de l’usure et des risques nous permet d’adapter finement les contrats. C’est un vrai plus pour la maîtrise technique et commerciale.',
      job: 'Responsable Souscription Habitation – Groupe IARD',
    },
  ],
  analyse: {
    title: 'Analyse automatisée de toitures par intelligence artificielle',
    information: [
      {
        title: 'Pourquoi le diagnostic toiture terrasse PDF est essentiel ?',
        description: 'Il offre un document technique clair et archivable pour suivre l’évolution du toit.',
      },
      {
        title: 'Comment se déroule le diagnostic toiture terrasse PDF ?',
        description: 'Inspection sur site, relevés techniques, analyses visuelles et rédaction d’un rapport numérique.',
      },
      {
        title: 'Que faire après diagnostic toiture terrasse PDF ?',
        description: 'Exploiter le rapport pour décider des réparations, programmer un suivi ou planifier une rénovation.',
      },
    ],
    explication:
      ' Détection, qualification et recommandation à partir d’images aériennes HD. En un clic, obtenez un diagnostic métier précis sans monter sur le toit.',
  },
  theyTrustUs: { title: 'Ils nous font confiance' },
  faq: {
    title1: 'FAQ',
    q1: { question: 'Question 1', answer: 'Réponse 1' },
    q2: { question: 'Question 2', answer: 'Réponse 2' },
    q3: { question: 'Question 3', answer: 'Réponse 3' },
    q4: { question: 'Question 4', answer: 'Réponse 4' },
    q5: { question: 'Question 5', answer: 'Réponse 5' },
    q6: { question: 'Question 6', answer: 'Réponse 6' },
    title2: 'Vous êtes un particulier , vous souhaitez trouver un couvreur ?',
  },
  roofDiagnostics: {
    title: 'Comprendre les enjeux long terme de diagnostic toiture terrasse PDF',
    description1:
      'Un diagnostic toiture terrasse PDF permet de disposer d’un support fiable et structuré, facilitant le suivi des interventions dans le temps. Ce rapport numérique regroupe les constatations sur l’étanchéité, l’isolation et la stabilité des éléments porteurs. Sur le long terme, il devient un outil de référence pour anticiper les travaux nécessaires, comparer l’évolution de l’état du toit et justifier les décisions prises auprès des assurances ou lors de transactions immobilières. ',
    description2:
      'En centralisant les informations dans un format accessible, il contribue à une meilleure gestion patrimoniale et à la pérennité de la toiture terrasse.',
  },
  footer: {
    title: 'Votre analyse Birdia près de chez vous',
    description: 'Découvrez nos offres disponibles dans les villes suivantes :',
    link: [
      { name: 'paris', route: '/paris' },
      { name: 'Lille', route: '/lille' },
      { name: 'Lyon', route: '/Lyon' },
      { name: 'paris', route: '/paris' },
      { name: 'paris', route: '/paris' },
      { name: 'paris', route: '/paris' },
      { name: 'paris', route: '/paris' },
      { name: 'paris', route: '/paris' },
      { name: 'paris', route: '/paris' },
    ],
  },
};
